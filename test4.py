...
class MyMainWindow(QMainWindow):
    ...
    def _build_splitter(self):
        ...
        # Create scroll area for main web view (chart pane)
        ...
        # 1) add the scroll area first (fills most of the space)
        main_web_layout.addWidget(self.main_web_scroll_area)

        # 2) external horizontal bar (MAIN) — now with ASYMMETRIC spacer widgets
        self.main_top_scrollbar = QScrollBar(Qt.Orientation.Horizontal)
        self.main_top_scrollbar.setObjectName("mainTopBar")
        self.main_top_scrollbar.setMaximumHeight(20)

        main_row = QHBoxLayout()
        main_row.setContentsMargins(0, 0, 0, 0)  # Remove margins
        main_row.setSpacing(0)

        # Left spacer (larger for visual alignment)
        main_left_spacer = QWidget()
        main_left_spacer.setFixedWidth(1300)  # Moderate left space
        main_row.addWidget(main_left_spacer)

        # The scrollbar (stretch = 1 so it grows in between spacers)
        main_row.addWidget(self.main_top_scrollbar, 1)

        # Right spacer (much smaller to prevent over-extension)
        main_right_spacer = QWidget()
        main_right_spacer.setFixedWidth(600)  # Very small right space
        main_row.addWidget(main_right_spacer)

        main_web_layout.addLayout(main_row)
        ...
        # Proximity line chart container
        self.web_page = QWidget()
        web_layout = QVBoxLayout(self.web_page)
        web_layout.setContentsMargins(0, 0, 0, 0)
        web_layout.setSpacing(0)

        # TOP scrollbar (PROX) — with ASYMMETRIC spacer widgets
        self.top_scrollbar = QScrollBar(Qt.Orientation.Horizontal)
        self.top_scrollbar.setObjectName("proxTopBar")
        self.top_scrollbar.setMaximumHeight(20)

        prox_row = QHBoxLayout()
        prox_row.setContentsMargins(0, 0, 0, 0)  # Remove margins
        prox_row.setSpacing(0)

        # Left spacer (same as main for alignment)
        prox_left_spacer = QWidget()
        prox_left_spacer.setFixedWidth(20)  # Same as main
        prox_row.addWidget(prox_left_spacer)

        prox_row.addWidget(self.top_scrollbar, 1)

        # Right spacer (small to prevent extension)
        prox_right_spacer = QWidget()
        prox_right_spacer.setFixedWidth(1800)  # Small right space
        prox_row.addWidget(prox_right_spacer)

        web_layout.addLayout(prox_row)
        ...
        # Apply scrollbar theme to both horizontal bars
        ...
        def _on_splitter_moved(*_):
            ...
            if self._hscroll_ready_main:
                _apply_main_fixed_range()
        # After creating main_row layout (for main scrollbar)
        def print_scrollbar_dimensions():
            # Get the parent widget that contains the scrollbar layout
            main_container = self.main_web_page
            total_width = main_container.width()
            
            # Get individual component widths
            left_spacer_width = main_left_spacer.width()
            right_spacer_width = main_right_spacer.width()
            scrollbar_width = self.main_top_scrollbar.width()
            
            print(f"=== Main Scrollbar Dimensions ===")
            print(f"Total container width: {total_width}px")
            print(f"Left spacer: {left_spacer_width}px")
            print(f"Actual scrollbar: {scrollbar_width}px") 
            print(f"Right spacer: {right_spacer_width}px")
            print(f"Sum: {left_spacer_width + scrollbar_width + right_spacer_width}px")

        # Call this after the widget is shown
        QTimer.singleShot(1000, print_scrollbar_dimensions)
    ...
    def _apply_scrollbar_theme(self, _accent_ignored="#b8b8b8"):
        ...
        # ---- HORIZONTAL: only our two top bars ----
        h_style = f"""
        QScrollBar#proxTopBar:horizontal,
        QScrollBar#mainTopBar:horizontal {{
            height:{bar_h}px;
            background: transparent;
            /* reserve left/right space so handle never slides under arrows */
            margin: 0 {btn_wh + 8}px 0 {btn_wh + 8}px;
        }}
        ...
        QScrollBar#mainTopBar::handle:horizontal:hover {{ background:#bfbfbf; border-color:rgba(0,0,0,0.28); }}
        QScrollBar#proxTopBar::handle:horizontal:pressed,
        QScrollBar#mainTopBar::handle:horizontal:pressed {{ background:#9a9a9a; border-color:rgba(0,0,0,0.38); }}

        /* arrows live inside the margin area */
        ...
        QScrollBar#mainTopBar::add-line:horizontal:hover {{ background:#d6d6d6; }}
        ...
        QScrollBar#mainTopBar::add-line:horizontal:pressed {{ background:#c2c2c2; }}

        /* keep "pages" transparent so only the track behind shows */
        ...
        QScrollBar#mainTopBar::sub-page:horizontal {{ background: transparent; }}
        """
