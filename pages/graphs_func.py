...
def plot_erf(df, view="Both", return_fig=False):
    ...
    if view in ["Internal", "Both"]:
        ...
        fig.add_trace(go.Scatter(
            ...
            name="Internal",
            mode="markers",
            marker=dict(symbol='triangle-up', color='steelblue', size=8),
            showlegend=True
        ))
    ...
    if view in ["External", "Both"]:
        ...
        fig.add_trace(go.Scatter(
            ...
            name="External",
            mode="markers",
            marker=dict(symbol='triangle-up', color='orangered', size=8),
            showlegend=True
        ))
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="Distance from Launcher (ODO) in m",
                   dtick=500, tickformat="~d", automargin=True),
        yaxis=dict(title="ERF (ASME B31G)",
                   range=[0, y_max], dtick=0.1,
...
#         xaxis=dict(title="Distance from Launcher (ODO) in m",
#                    dtick=500, tickformat="~d", automargin=True),
#         yaxis=dict(title="Psafe (ASME B31G) in kg/cm²",
#                    range=[0, y_max], dtick=50, automargin=True),
#         template="plotly_white"
#     )
#
...
def plot_psafe(df, view="Both", return_fig=False):
    ...
    temp_dir = tempfile.gettempdir()   # ✅ temp folder define

    df.columns = df.columns.str.strip()

    if 'Psafe (ASME B31G) Barg' not in df.columns:
        raise ValueError("Required column 'Psafe (ASME B31G) Barg' not found in dataframe.")

    df = df.rename(columns={
        'Psafe (ASME B31G) Barg': 'Psafe (ASME B31G)'
    })
    ...
    html_path = os.path.join(temp_dir, "psafe_plot.html")   # ✅ now it works
    fig.write_html(html_path)
    return fig, html_path
...
# def plot_depth(df, return_fig=False, selected_bin="All"):
#     df.columns = df.columns.str.strip()
#
...
#
#     if selected_bin != "All":
#         df = df[df['Depth Bin'] == selected_bin]
#
#     # Group and count
...
#
#     ax.view_init(elev=25, azim=-75)
#     plt.title("Distribution of Metal Loss Depth Across Launcher Distance", pad=30, fontsize=14)
#     plt.tight_layout()
#
#     if return_fig:
...
def plot_depth(df, return_fig=False, selected_bin="All"):
    ...
    df['Abs. Distance (m)'] = df['Abs. Distance (m)'].round(1)
    df = df.groupby(['Abs. Distance (m)', 'Surface Location'], as_index=False)\
           .agg({'Depth % WT': 'max'})
    ...
    # Group and count (safe)
    grouped = df.groupby(['Distance Bin', 'Depth Bin']).size().unstack(fill_value=0)

    # If nothing is left, build empty grouped with same bin structure
    if grouped.empty:
        grouped = pd.DataFrame(
            0,
            index=pd.CategoricalIndex([], categories=pd.cut([], bins=distance_edges).categories),
            columns=[selected_bin] if selected_bin != "All" else depth_labels
        )
    ...
    dx = dy = 0.4
    depth_colors = {'0–20%': 'steelblue', '20–40%': 'darkorange',
                    '40–60%': 'seagreen', '60–80%': 'yellow'}
    colors = [depth_colors[y_labels[y]] for y in ypos]
    ...
    if return_fig:
        # ✅ Save PNG to temp folder for displaying in QWebEngineView
        import tempfile, os
        temp_dir = tempfile.gettempdir()
        output_path = os.path.join(temp_dir, "depth_plot.png")
        fig.savefig(output_path, dpi=300)
        plt.close(fig)  # close to prevent extra window
        return fig, output_path
    else:
        plt.show()
        return None, None
...
def plot_orientation(df, view="Both", return_fig=False):
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="Distance from Launcher (ODO) in m",
                   dtick=500, tickformat="~d", automargin=True),
        yaxis=dict(title="Circumferential Orientation (o'clock)",
                   tickvals=[i * 30 for i in range(13)],
                   ticktext=[f"{i:02}:00:00" for i in range(13)],
                   automargin=True),
        template="plotly_white"
    )
