...
class PipeHighlightEmbedded(QWidget):
    def __init__(self, parent=None, pipe_tally_df=None, project_root=None):
        ...
        if not self.pipe_tally_df.empty:
            ...
            print("⚠ No pipe tally data provided")
    ...
    def _initialize_constants(self):
        ...
        if os.path.exists(constants_file):
            try:
                ...
                print(f"⚠ Failed to load constants.xlsx: {e}")

        # Fallback defaults if Excel missing or invalid
        print("⚠ Using fallback default constants")
        self.CLIENT = 'ABC'
        self.CONTRACTOR = 'ZZZ'
        self.IP_TYPE = 'MFL'
        self.MEDIUM = 'Oil'
    ...
    def _calculate_statistics(self):
        ...
        if self.pipe_tally_df.empty:
            print("⚠ DataFrame is empty, using default values")
            self.TOT_ANAL = 0
            self.INT_ANAL = 0
            self.EXT_ANAL = 0
            self.ERF_95 = 0
        ...
        try:
            ...
            if depth_col:
                ...
                if len(depth_data) > 0:
                    ...
                    self.DEPTH_DIST = depth_bins.value_counts().sort_index().to_dict()
                else:
                    self.DEPTH_DIST = {f"{i + 1}-{i + 10}" if i > 0 else "0-10": 0 for i in range(0, 100, 10)}
            else:
                self.DEPTH_DIST = {f"{i + 1}-{i + 10}" if i > 0 else "0-10": 0 for i in range(0, 100, 10)}
    ...
    def _setup_ui(self):
        ...
        content_widget = QWidget()
        content_widget.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        content_widget.setMinimumHeight(2500)
    ...
    def _create_general_info_section(self, parent_layout):
        ...
        info_items = [
            ...
            # col 4-5
            [("➖➖ PipeLine Section", self.PIPELINE_SECTION),
            ("🔺 Design Pressure (kg/cm²):", str(self.DP)),
            ("💪 UTS (MPa):", str(self.UTS))],

            # next row
            [("⚙ Type of Pipe:", self.TYPE_PIPE),
            ("⚡ Operating Pressure (kg/cm²):", str(self.OP)),
            ("🔩 SMYS (MPa):", str(self.SMYS))],
    ...
    def _create_statistics_section(self, parent_layout):
        ...
        colors = ["#8b5cf6", "#10b981", "#f59e0b", "#ef4444", "#3b82f6",
                  "#06b6d4", "#f87171", "#facc15", "#22c55e", "#ec4899"]

        for idx, (label, count) in enumerate(self.DEPTH_DIST.items()):
            depth_stats.append((f"Depth {label}%", count, colors[idx % len(colors)]))

        self._create_stats_group(stats_layout, "📊 Depth Distribution", depth_stats)
    ...
    def _create_charts(self):
        ...
        try:
            ...
            axs[0].set_title(' ⚠️ Internal vs External Anomalies', fontsize=16, fontweight='bold',
                           pad=35, color='#1f2937')
            ...
            axs[1].set_title('⚡ Estimate Repair Factor (ERF) Analysis', fontsize=16, fontweight='bold',
                           pad=15, color='#1f2937')

            # # ✨ Chart 3: Depth Distribution
            labels_3 = list(self.DEPTH_DIST.keys())
            sizes_3 = list(self.DEPTH_DIST.values())

            premium_colors_3 = ['#8b5cf6', '#10b981', '#f59e0b', '#dc2626',
                                '#3b82f6', '#06b6d4', '#f87171', '#facc15',
                                '#22c55e', '#ec4899']
