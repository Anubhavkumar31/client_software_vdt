...
USE_HARDCODED_POINTS = False    # Set to True to use hardcoded test points
SAVE_DF_SENS_CSV = False         # Set to True to save df_sens as CSV file
INTERACTIVE_MODE = False         # Set to True for interactive plotly window, False for browser HTML

# Hardcoded test points for overlay markers (x, y_band, label)
HARDCODED_POINTS = [
    (620.0, "00:00:00", "Test1"),    # Point at x=620m, 12 o'clock position
    (622.5, "03:00:00", "Test2"),    # Point at x=622.5m, 3 o'clock position  
    (625.0, "06:00:00", "Test3"),    # Point at x=625m, 6 o'clock position
    (627.5, "09:00:00", "Test4"),    # Point at x=627.5m, 9 o'clock position
    (624.0, "01:30:00", "Test5"),    # Point at x=624m, 1:30 position
]
# -----------------------------------------------------------------
...
def pre_process_for_heatmap(df_in: pd.DataFrame, pipe_number: Union[str, int] = None):
    ...
    # ---- Save df_sens as CSV if enabled ----
    if SAVE_DF_SENS_CSV:
        csv_filename = f"df_sens_pipe_{pipe_number if pipe_number else 'unknown'}.csv"
        try:
            df_sens_raw.to_csv(csv_filename, index=True)
            print(f"SUCCESS: Saved raw sensor data to: {csv_filename}")
            
            processed_filename = f"df_sens_processed_pipe_{pipe_number if pipe_number else 'unknown'}.csv"
            df_sens_plot.to_csv(processed_filename, index=True)
            print(f"SUCCESS: Saved processed sensor data to: {processed_filename}")
        except Exception as e:
            print(f"WARNING: Failed to save df_sens CSV: {e}")
...
def render_heatmap_in_browser(df_sens_final: pd.DataFrame,
    ...
    if USE_HARDCODED_POINTS:
        print("DEBUG: Using hardcoded test points for overlay")
        for point in HARDCODED_POINTS:
            x, y_band, label = point
    ...
    if INTERACTIVE_MODE:
        print("=" * 60)
        print("INTERACTIVE MODE: Opening Plotly window...")
        print("Available interactions:")
        print("- Hover: Show exact values and coordinates")
        print("- Zoom: Mouse wheel, box zoom, or double-click")
        print("- Pan: Click and drag")
        print("- Reset: Double-click to reset view")
        print("- Download: Camera icon to save as PNG")
        print("- Close browser tab when finished")
        print("=" * 60)
        
        fig.show()
        return
    
    # Browser mode - save to interactive HTML
    print("BROWSER MODE: Creating interactive HTML file...")
    
    # Save as interactive HTML
    html_file = f"interactive_heatmap_plotly_pipe_{pipe_number}.html"
    plot(fig, filename=html_file, auto_open=True, config={
        'displayModeBar': True,
        'displaylogo': False,
...
if __name__ == "__main__":
    ...
    print(f"Use Hardcoded Points: {USE_HARDCODED_POINTS}")
