...
from PyQt6.QtCore import (
    ...
    QTimer, QUrl, QEvent, QAbstractTableModel, QModelIndex
)
from PyQt6.QtGui import (
    QStandardItemModel, QStandardItem, QMovie, QAction, QCursor
)
from PyQt6.QtWidgets import (
    QApplication, QMainWindow, QFileDialog, QHeaderView, QLabel, QSizePolicy,
    QTableWidget, QTableWidgetItem, QStatusBar, QVBoxLayout, QWidget, QHBoxLayout,
    QMessageBox, QDialog, QTextEdit, QPushButton, QSplitter, QStackedWidget,
...
class DataFrameModel(QAbstractTableModel):
    ...
    def headerData(self, section, orientation, role=Qt.ItemDataRole.DisplayRole):
        ...
        if orientation == Qt.Orientation.Horizontal:
            return str(self._df.columns[section]) if self._df is not None and section < len(self._df.columns) else ""
...
# ---------- pop-out full-table viewer ----------
class FullTableDialog(QDialog):
    def __init__(self, df: pd.DataFrame, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Full Table")
        self.resize(1400, 900)
        lay = QVBoxLayout(self)

        # search
        top = QHBoxLayout()
        self.edit = QLineEdit(self); self.edit.setPlaceholderText("Filter (matches any column)…")
        btn_fit = QPushButton("Autofit columns", self)
        btn_export = QPushButton("Export CSV", self)
        top.addWidget(self.edit); top.addStretch(1); top.addWidget(btn_fit); top.addWidget(btn_export)
        lay.addLayout(top)
        ...
        self.edit.textChanged.connect(self._proxy.setFilterFixedString)
        btn_fit.clicked.connect(self.autofit)
        btn_export.clicked.connect(self.export_csv)

    def setFrame(self, df: pd.DataFrame):
        self._model.setFrame(df)
        self.autofit()
    ...
    def autofit(self):
        ...
        for c in range(self._model.columnCount()):
            w = max(120, self.view.columnWidth(c))
    ...
    def export_csv(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Export table", "table.csv", "CSV (*.csv)")
        if not path: return
        try:
...
# ---------- splitter handle with mid tabbar ----------
class MidBarHandle(QSplitterHandle):
    def __init__(self, orientation, parent, tabbar: QTabBar):
        ...
        self.frame = QFrame(self); self.frame.setObjectName("MidBarFrame")
        self.frame.setFrameShape(QFrame.Shape.NoFrame); self.frame.setCursor(Qt.CursorShape.SplitVCursor)
        self.tabbar = tabbar; self.tabbar.setParent(self.frame); self.tabbar.setDrawBase(False)
        self.tabbar.setCursor(Qt.CursorShape.ArrowCursor); self.tabbar.setMouseTracking(True)
        self.tabbar.setAttribute(Qt.WidgetAttribute.WA_Hover, True)
        lay = _QHBoxLayout(self.frame); lay.setContentsMargins(8, 4, 8, 4); lay.addWidget(self.tabbar)
    ...
    def eventFilter(self, obj, ev):
        if obj is self.tabbar:
            ...
            if t in (QEvent.Type.MouseMove, QEvent.Type.HoverMove):
                ...
                self.tabbar.setCursor(Qt.CursorShape.ArrowCursor); return False
            if p is not None:
                idx = self.tabbar.tabAt(p)
                self.tabbar.setCursor(Qt.CursorShape.PointingHandCursor if idx != -1 and self.tabbar.isTabEnabled(idx) else Qt.CursorShape.ArrowCursor)
            return False
...
class MainApp(QApplication):
    def __init__(self, sys_argv):
        ...
        self.splash = None; self.main_window = None
    ...
    def show_splash_screen(self):
        ...
        if label:
            ...
            self.movie = QMovie(gif_path); label.setMovie(self.movie); self.movie.start()
        self.splash.show()
    ...
    def show_main_window(self):
        self.main_window = MyMainWindow(); self.main_window.show()

    def start(self):
        self.show_splash_screen()
        self.timer = QTimer(self); self.timer.setSingleShot(True)
        self.timer.timeout.connect(self.initialize_app); self.timer.start(1200)

    def initialize_app(self):
        self.close_splash_screen(); self.show_main_window()


class MyMainWindow(QMainWindow):
    REQUIRED_TALLY_COLS = [r"Abs. Distance (m)", r"Depth %", r"Type", r"ERF (ASME B31G)", r"Orientation o' clock"]
    ...
    def __init__(self):
        ...
        self._central_graphs = None; self._graphs_widget = None

        self.project_is_open = False; self.project_root = None; self.pkl_files = []
        self.curr_data = None; self.header_list = []; self.pipe_tally = None; self.prox_linechart = None
        self.hmap = self.hmap_r = self.lplot = self.lplot_r = self.pipe3d = self.heatmap_box = None

        self._reverting_tab = False; self._last_allowed_tab_index = 0; self._ui_ready = False

        self.defect_df_view: pd.DataFrame | None = None  # <— stored copy for the pop-out dialog

        self.ui.comboBoxPipe.setEditable(True); self.ui.comboBoxPipe.clear(); self.ui.comboBoxPipe.addItem("-Pipe-")
        self.ui.comboBoxPipe.setMaxVisibleItems(12)
        self.ui.comboBoxPipe.completer().setCompletionMode(QtWidgets.QCompleter.CompletionMode.PopupCompletion)
        self.ui.comboBoxPipe.setInsertPolicy(QComboBox.InsertPolicy.NoInsert)

        self.model = QStandardItemModel(self); self.proxy_model = QSortFilterProxyModel(self)
        self.proxy_model.setSourceModel(self.model); self.ui.tableView.setModel(self.proxy_model)

        # Buttons near the pipe combo
        ...
        self.btnFullTable = QPushButton("Full Table")      # NEW
        self.btnFullTable.setToolTip("Open the defect table in a full-screen pop-out.")
        self.btnFullTable.setEnabled(False)
        self.btnFullTable.clicked.connect(self.open_full_table)

        try:
            _parent = self.ui.comboBoxPipe.parentWidget(); _lay = _parent.layout()
            if _lay is not None:
                pos = _lay.indexOf(self.ui.comboBoxPipe)
                if pos != -1:
                    _lay.insertWidget(pos + 1, self.btnDigsheetAbs)
                    _lay.insertWidget(pos + 2, self.btnFullTable)
                else:
                    _lay.addWidget(self.btnDigsheetAbs); _lay.addWidget(self.btnFullTable)
            else:
                self.btnDigsheetAbs.setParent(_parent); self.btnFullTable.setParent(_parent)
        except Exception:
            self.statusBar().addPermanentWidget(self.btnDigsheetAbs)
            self.statusBar().addPermanentWidget(self.btnFullTable)

        self.btnDigsheetAbs.clicked.connect(self.open_digsheet_by_abs_from_selection)

        QtWidgets.QApplication.instance().installEventFilter(self)

        # mid tabbar mirrors tabWidgetM
        ...
        # Make the bottom defect table behave like a real grid
        ...
        self._apply_defect_table_prefs()  # <— force scrollbars/interactive headers
        ...
        right_container = QWidget(); rl = QHBoxLayout(right_container); rl.setContentsMargins(0,0,0,0)
    ...
    # ----- force scrollbars & headers on the bottom table -----
    def _apply_defect_table_prefs(self):
        tw = self.ui.tableWidgetDefect
        tw.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOn)
        tw.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOn)
        tw.setHorizontalScrollMode(QAbstractItemView.ScrollMode.ScrollPerPixel)
    ...
    def _connect_guarded_graph_controls(self):
        a = self.ui
        for aname, tab in (("actionHeatmap","Heatmap"),("action_LineChart","LineChart"),("action_3D_Graph","3D")):
        ...
        for wname, tab in (("btnHeatmap","Heatmap"),("toolButtonHeatmap","Heatmap"),
                           ("btnLinechart","LineChart"),("toolButtonLine","LineChart"),
                           ("btn3D","3D"),("toolButton3D","3D")):
    ...
    def _guarded_open_tab(self, tab_name: str):
        if not self.project_is_open:
            if self._ui_ready: self._project_required_popup()
            return
        wanted = {"Heatmap":{"Heatmap"},"LineChart":{"LineChart","Line Chart","Line Plot"},"3D":{"3D Graph","3D"}}.get(tab_name,{tab_name})
        tw = self.ui.tabWidgetM
        for i in range(tw.count()):
            if tw.tabText(i) in wanted:
                tw.setCurrentIndex(i); self.tab_switcher2(); return
    ...
    def _build_splitter(self):
        ...
        self.ui.tableWidgetDefect.setParent(self.defect_table_page); dl.addWidget(self.ui.tableWidgetDefect)
        ...
        self.ui.tableView.setParent(self.data_table_page); tl.addWidget(self.ui.tableView)
        ...
        self.splitter.setChildrenCollapsible(False); self.splitter.setHandleWidth(40)
        ...
        self.splitter.setStyleSheet("""
            ...
            QTabBar::tab { color: #d8d8d8; padding: 6px 14px; margin: 0; border: 0; background: transparent; }
    ...
    def _show_watermark(self):
        ...
        self.bottom_stack.setCurrentIndex(0); self.web_view2.setUrl(QUrl())

    def _tick(self):
        if getattr(self, "_t0", None):
            dt = time.time() - self._t0; self.right_status_label.setText(f"{dt:.1f}s    ")

    def set_loading(self, msg="Loading"):
        self.current_message = msg; self.statusBar().showMessage(f'           Status:      {self.current_message}')
        self._t0 = time.time(); self.timer.start(100)

    def set_idle(self):
        self.current_message = 'App running'; self.statusBar().showMessage(f'           Status:      {self.current_message}')
        self.timer.stop(); self._t0 = None; self.right_status_label.setText("0.0s    ")
    ...
    def setup_actions(self):
        ...
        a.actionStandard.triggered.connect(self.open_digs)

    # ---------- project open ----------
    def open_project(self):
        try:
            dlg = QFileDialog(self); dlg.setFileMode(QFileDialog.FileMode.Directory)
            dlg.setOption(QFileDialog.Option.ShowDirsOnly); dlg.setWindowTitle("Select Project Folder (PKLs + pipe_* folders)")
            if dlg.exec() != QFileDialog.DialogCode.Accepted:
                self.project_is_open = False; self._toggle_plot_ui(False); self._show_watermark(); self._update_project_actions(); return

            root = dlg.selectedFiles()[0]; self.project_root = root
            self.pipe_tally = None; loaded_tally = self._auto_load_pipe_tally(root)
            if not loaded_tally: print("[pipe_tally] No tally file found in this project; graphs/reports will warn if needed.")

            self.pkl_files = [os.path.join(root, f) for f in os.listdir(root) if f.lower().endswith(".pkl")]
            def nkey(path):
                filename = os.path.basename(path)
                return [int(t) if t.isdigit() else t.lower() for t in re.split(r"(\d+)", filename)]
            self.pkl_files.sort(key=nkey)

            cb = self.ui.comboBoxPipe; cb.blockSignals(True); cb.clear()
            names = [os.path.splitext(os.path.basename(f))[0] for f in self.pkl_files]
            if names: cb.addItems(names); cb.setCurrentIndex(0)
            ...
            cb.setInsertPolicy(QtWidgets.QComboBox.InsertPolicy.NoInsert); cb.blockSignals(False)
            try: cb.lineEdit().returnPressed.disconnect()
            except Exception: pass
            cb.lineEdit().returnPressed.connect(self.jump_to_number)

            if self.pkl_files:
                self.project_is_open = True; self._toggle_plot_ui(True); self.load_selected_by_index(0)
            else:
                self.project_is_open = False; self._toggle_plot_ui(False); self._show_watermark()
                QMessageBox.warning(self, "No PKLs", "No .pkl files found in the selected folder.")

            self._update_project_actions()
        except Exception as e:
            self.project_is_open = False; self._toggle_plot_ui(False); self._show_watermark(); self._update_project_actions()
            self.open_Error(e)

    def gen_data(self):
        try:
            if 'genData' not in self.child_windows or not self.child_windows['genData'].isVisible():
                self.script_runner_window = ScriptRunnerApp(); self.script_runner_window.show()
                self.child_windows['genData'] = self.script_runner_window
            else:
                self.child_windows['genData'].raise_(); self.child_windows['genData'].activateWindow()
        except Exception as e:
            self.open_Error(e)
    ...
    def _toggle_plot_ui(self, enabled: bool):
        ...
        self.btnFullTable.setEnabled(bool(enabled and self.defect_df_view is not None and not self.defect_df_view.empty))

    def on_combo_index_changed(self, combo_idx: int):
        if not self.project_is_open or combo_idx < 0: return
        self.load_selected_by_index(combo_idx)

    def load_selected_by_index(self, idx: int):
        try:
            if idx < 0 or idx >= len(self.pkl_files): return
            pkl_path = self.pkl_files[idx]
            df = pd.read_pickle(pkl_path); self.curr_data = df; self.header_list = list(df.columns)
            self.model.clear(); self.model.setHorizontalHeaderLabels([str(c) for c in df.columns])
            for _, row in df.iterrows():
                self.model.appendRow([QStandardItem(str(v)) for v in row.values])
            self.ui.tableView.setModel(self.model); self.ui.tableView.setSortingEnabled(True)

            name = os.path.splitext(os.path.basename(pkl_path))[0]; pipe_idx = self._extract_index(name)
    ...
    def _extract_index(text: str) -> str:
        m = re.search(r'\d+', text); return m.group(0) if m else text

    def load_assets_for_index(self, pipe_idx: str):
        try:
            if not (self.project_root and self.project_is_open): return

            candidates = [os.path.join(self.project_root, f"pipe_{pipe_idx}"),
            ...
            if not pipe_dir: return self.open_Error(f"Folder not found for pipe index {pipe_idx} (looked for pipe_{pipe_idx}).")

            def pick_one(patterns, exclude=None):
                exclude = exclude or []; hits = []
                for pat in patterns: hits.extend(glob(os.path.join(pipe_dir, pat)))
            ...
            if pipe_tally_csv:
                try:
                    df = pd.read_csv(pipe_tally_csv) if pipe_tally_csv.lower().endswith(".csv") else pd.read_excel(pipe_tally_csv)
                ...
                if ds_csv:
                    try:
                        self._populate_defect_table_from_csv(pd.read_csv(ds_csv))
                    except Exception as e:
    ...
    # ---------- write defect table from PipeTally ----------
    def _populate_defect_table_from_tally(self, df: pd.DataFrame):
        ...
        desired_cols = [
            ...
            "Feature Identification","Feature Type","Dimensions Classification","Orientation o' clock",
            "Length (mm)","Width (mm)","WT (mm)","Depth %","Depth (mm)","Type",
            "ERF (ASME B31G)","Psafe (ASME B31G) Barg","Latitude","Longitude","Comment"
        ]
        variants = {"s_no":"Defect_id","Dimensions  Classification":"Dimensions Classification","Depth % ":"Depth %",
                    "Psafe (ASME B31G) bar":"Psafe (ASME B31G) Barg","Pipe Length":"Pipe Length (mm)",
                    "Length":"Length (mm)","Width":"Width (mm)","WT":"WT (mm)"}
        for src, dst in variants.items():
            if src in df.columns and dst not in df.columns: df[dst] = df[src]
        if "Defect_id" not in df.columns:
            df = df.reset_index(drop=True); df["Defect_id"] = np.arange(1, len(df)+1)
        for col in desired_cols:
            if col not in df.columns: df[col] = ""

        view = df[desired_cols].copy()
        self.defect_df_view = view.copy()  # store for pop-out

        tw = self.ui.tableWidgetDefect
        tw.clear(); tw.setRowCount(len(view)); tw.setColumnCount(len(view.columns))
        tw.setHorizontalHeaderLabels([str(c) for c in view.columns])

        for r in range(len(view)):
            for c, col in enumerate(view.columns):
                v = view.iloc[r, c]
                tw.setItem(r, c, QTableWidgetItem("" if pd.isna(v) else str(v)))

        self._apply_defect_table_prefs()
        self.btnFullTable.setEnabled(True)
        self.update_digsheet_button_state()

    # ---------- write defect table from legacy CSV ----------
    def _populate_defect_table_from_csv(self, df: pd.DataFrame):
        header_indices = {'Defect_id':0,'Absolute_Distance':1,'Upstream_Distance':2,'Feature_Type':3,
                          'Dimension_Class':4,'Orientation':5,'WT':6,'Length':7,'Width':8,'Depth_Peak':9}
        colmap = {'Box Number':'Defect_id','Defect_id':'Defect_id','Absolute Distance':'Absolute_Distance',
                  'Abs. Distance (m)':'Absolute_Distance','Upstream':'Upstream_Distance',
                  'Distance to U/S GW(m)':'Upstream_Distance','Type':'Feature_Type',
                  'Dimensions  Classification':'Dimension_Class',"Orientation o' clock":'Orientation',
                  'Ori Val':'Orientation','WT (mm)':'WT','WT':'WT','Width':'Length','Breadth':'Width',
                  'Peak Value':'Depth_Peak','Depth % ':'Depth_Peak','Depth %':'Depth_Peak','Length':'Length'}

        # build normalized DataFrame in the header order
        view = pd.DataFrame(columns=list(header_indices.keys()), index=range(len(df)))
        ...
        self.defect_df_view = view.copy()

        tw = self.ui.tableWidgetDefect
        tw.setRowCount(len(view)); tw.setColumnCount(len(view.columns))
        tw.setHorizontalHeaderLabels(list(view.columns))
        for r in range(len(view)):
            for c, col in enumerate(view.columns):
                v = view.iloc[r, c]; tw.setItem(r, c, QTableWidgetItem("" if pd.isna(v) else str(v)))
    ...
    def _on_middle_tab_changed(self, index: int):
        if self._reverting_tab: return
        if not self.project_is_open:
            if self._ui_ready: self._project_required_popup()
            self._reverting_tab = True
            try: self.ui.tabWidgetM.setCurrentIndex(self._last_allowed_tab_index)
            finally: self._reverting_tab = False
            return
        self._last_allowed_tab_index = index; self.tab_switcher2(); self.update_digsheet_button_state()

    def tab_switcher2(self, *_):
        if not self.project_is_open:
            self._show_watermark(); return
        try:
            tab = self.ui.tabWidgetM.tabText(self.ui.tabWidgetM.currentIndex())
            if tab == "Heatmap":
                self._load_scrollable_chart(self.web_view, self.hmap, 2200, 1400) if self.hmap else self.web_view.setUrl(QUrl())
                self.bottom_stack.setCurrentIndex(0); self.web_view2.setUrl(QUrl())
            elif tab in ("LineChart","Line Chart","Line Plot"):
                self._load_scrollable_chart(self.web_view, self.lplot, 2200, 1400) if self.lplot else self.web_view.setUrl(QUrl())
                if self.prox_linechart and os.path.exists(self.prox_linechart):
                    self.bottom_stack.setCurrentIndex(2); self._load_scrollable_chart(self.web_view2, self.prox_linechart, 2200, 900)
                else:
                    self.bottom_stack.setCurrentIndex(0); self.web_view2.setUrl(QUrl())
            elif tab in ("3D Graph","3D"):
                if self.pipe3d:
                    try: self._load_scrollable_chart(self.web_view, self.pipe3d, 2200, 1400)
                    except AttributeError: self.web_view.setUrl(QUrl.fromLocalFile(self.pipe3d))
                else: self.web_view.setUrl(QUrl())
                self.bottom_stack.setCurrentIndex(0); self.web_view2.setUrl(QUrl())
            self.update_digsheet_button_state()
        except Exception as e:
            self.open_Error(e)
    ...
    def _load_scrollable_chart(self, view: QWebEngineView, html_path: str, min_w: int = 2200, min_h: int = 1400):
        if not html_path or not os.path.exists(html_path):
            view.setUrl(QUrl()); return
        safe = html_path.replace('\\','/')
        wrapper = f"""<!doctype html><html><head><meta charset="utf-8"><style>
          html,body{{height:100%;margin:0}}.wrap{{height:100vh;width:100%;overflow:auto}}
          iframe{{border:0;width:{min_w}px;height:{min_h}px}}
        </style></head><body><div class="wrap"><iframe sandbox="allow-scripts allow-same-origin" src="file:///{safe}"></iframe></div></body></html>"""
        base = QUrl.fromLocalFile(os.path.dirname(html_path) + os.sep)
        view.setHtml(wrapper, base)

    # ---------- selection helpers + digsheet gating ----------
    def _abs_col_candidates(self): return ("Absolute_Distance","Abs. Distance (m)","Absolute Distance")

    def _abs_col_index_silent(self) -> Optional[int]:
        tw = self.ui.tableWidgetDefect
        if tw.columnCount()==0: return None
        for c in range(tw.columnCount()):
            hdr = tw.horizontalHeaderItem(c); name = hdr.text().strip() if hdr else ""
            if name in self._abs_col_candidates(): return c
        return 1 if tw.columnCount()>1 else (0 if tw.columnCount()==1 else None)

    def _has_valid_abs_selection(self) -> bool:
        tw = self.ui.tableWidgetDefect
        if tw.rowCount()==0 or tw.columnCount()==0: return False
        abs_col = self._abs_col_index_silent()
        if abs_col is None: return False
        sel_model = tw.selectionModel()
        rows = [i.row() for i in sel_model.selectedRows()] or [i.row() for i in tw.selectedIndexes()]
    ...
    def update_digsheet_button_state(self):
        can_show = (self.project_is_open and isinstance(self.pipe_tally, pd.DataFrame)
                    and self._is_graph_tab_ok() and self._has_valid_abs_selection())
        self.btnDigsheetAbs.setEnabled(bool(can_show))
        self.btnDigsheetAbs.setCursor(Qt.CursorShape.PointingHandCursor if can_show else Qt.CursorShape.ForbiddenCursor)
        self.btnDigsheetAbs.setToolTip("Click to generate Digsheet for the selected Absolute Distance."
                                       if can_show else "Select an Absolute Distance cell in the table below to enable.")

    def _get_selected_abs_distance_from_defect_table(self) -> Optional[str]:
        tw = self.ui.tableWidgetDefect
        if tw.rowCount() == 0 or tw.columnCount() == 0:
            QMessageBox.warning(self, "No data", "Defect table is empty."); return None
        abs_col = self._abs_col_index_silent()
        if abs_col is None:
            QMessageBox.warning(self, "Missing column", "Could not find the Absolute Distance column."); return None
        sel_model = tw.selectionModel()
        rows = [idx.row() for idx in sel_model.selectedRows()] or [i.row() for i in tw.selectedIndexes()]
        rows = list(dict.fromkeys(rows))
        if len(rows) != 1:
            QMessageBox.information(self, "Select one row", "Please select exactly one row in the defect table."); return None
        item = tw.item(rows[0], abs_col)
        if item is None or not item.text().strip():
            QMessageBox.warning(self, "No Absolute Distance", "Selected row has empty Absolute Distance."); return None
        return item.text().strip()

    # ---------- Full-table pop-out ----------
    def open_full_table(self):
        if self.defect_df_view is None or self.defect_df_view.empty:
            QMessageBox.information(self, "Full Table", "No table loaded yet."); return
        self._full_table_dlg = getattr(self, "_full_table_dlg", None)
        if self._full_table_dlg is None or not isinstance(self._full_table_dlg, FullTableDialog):
            self._full_table_dlg = FullTableDialog(self.defect_df_view, self)
        else:
            self._full_table_dlg.setFrame(self.defect_df_view)
        self._full_table_dlg.show(); self._full_table_dlg.raise_(); self._full_table_dlg.activateWindow()

    # ---------- other features (unchanged except tiny guards) ----------
    def draw_boxes_v2(self):
        if not self.project_is_open: return
        try:
    ...
    def minimize_tabs(self): self.ui.tabWidgetM.hide()
    def maximize_tabs(self): self.ui.tabWidgetM.show()

    def open_graphs(self):
        try:
            if self.pipe_tally is None: self.open_Error("Pipe tally not loaded yet."); return
            if self._central_graphs is not None and self.centralWidget() is self._central_graphs: return
            if self._central_original is None: self._central_original = self.centralWidget()
            ui_file_path = resource_path(os.path.join("ui","graphs_ui.py"))
            if not os.path.exists(ui_file_path): self.open_Error(f"Graphs UI file not found at:\n{ui_file_path}"); return
            import importlib.util
            spec = importlib.util.spec_from_file_location("graphs_ui", ui_file_path)
            graphs_ui = importlib.util.module_from_spec(spec); spec.loader.exec_module(graphs_ui)
            container = QWidget(); v = QVBoxLayout(container); v.setContentsMargins(12,12,12,12); v.setSpacing(10)
            header = QHBoxLayout(); back_btn = QPushButton("◀ Back"); back_btn.setCursor(Qt.CursorShape.PointingHandCursor)
            back_btn.clicked.connect(self._close_graphs_view); title = QLabel("Graphs"); title.setStyleSheet("font-weight:600;font-size:14pt;")
            header.addWidget(back_btn); header.addSpacing(12); header.addWidget(title); header.addStretch(1); v.addLayout(header)
            graphs_widget = graphs_ui.GraphApp(dataframe=self.pipe_tally); v.addWidget(graphs_widget, stretch=1)
            self._graphs_widget = graphs_widget; self._central_graphs = container
            if self._central_original is not None and self._central_original.parent() is self: self.takeCentralWidget()
            self.setCentralWidget(container)
        except Exception as e:
            try:
                if self.centralWidget() is None and self._central_original is not None: self.setCentralWidget(self._central_original)
            except Exception: pass
            self.open_Error(f"Unable to open graphs inline: {e}")

    def _close_graphs_view(self):
        try:
            if self.centralWidget() is self._central_original: return
            graphs_central = self.takeCentralWidget()
            if graphs_central is not None: graphs_central.deleteLater()
            if self._central_original is not None:
                if self._central_original.parent() is not self: self._central_original.setParent(self)
                self.setCentralWidget(self._central_original)
            self._graphs_widget = None; self._central_graphs = None
        except Exception as e:
            print("⚠️ _close_graphs_view:", e)

    def _auto_load_pipe_tally(self, root: str) -> bool:
        candidates = [os.path.join(root,"pipetally","pipe_tally.xlsx"),
        ...
        for d in (root, os.path.join(root,"pipetally")):
            if os.path.isdir(d):
                for f in os.listdir(d):
                    name = f.lower()
                    if name.endswith((".xlsx",".xls",".csv")): candidates.append(os.path.join(d,f))
        seen=set()
        for path in candidates:
            ...
            try:
                ...
                self.pipe_tally = df; return True
            except Exception as e:
                print(f"[pipe_tally] Failed to load {path}: {e}")
        self.pipe_tally = None; return False
    ...
    def open_Cluster(self): Cluster_Dialog().exec()
    def open_Ptal(self):
        try:
            file_path, _ = QFileDialog.getOpenFileName(self,"Open Pipe Tally File","","CSV/Excel Files (*.csv *.xlsx *.xls);;All Files (*)")
    ...
    def jump_to_number(self):
        ...
        try:
            ...
            if text in base_names: idx = base_names.index(text)
            else:
    ...
    def open_About(self): About_Dialog().exec()
    def open_Admin(self): self.ap = Admin_Panel(); self.ap.show()
    def open_Assessment(self): Assess_Dialog().exec()

    def open_PipeHigh(self):
        try:
            from pages.Pipe_Highlights import run_app; run_app()
        except Exception as e:
            self.open_Error(f"Error running Pipe Highlight:\n{e}")

    def open_PipeScheme(self):
        try:
            from pipeline_schema.pipeline_schema import run_app; run_app()
        except Exception as e:
    ...
    def open_Report(self):
        ...
        fil = self.pipe_tally[cols].copy(); fil = fil.dropna(subset=["Abs. Distance (m)"])
        ...
        fil["Surface Location"] = fil["Type"].apply(lambda x: "Internal" if "Internal" in x else ("External" if "External" in x else "Unknown"))
        self.fr = Report(fil); self.fr.show()

    def open_ERF(self):
        self.erf = ERF()
        def update_result():
            OD = self.erf.doubleSpinBox.value(); WT = self.erf.doubleSpinBox_3.value()
            SMYS = self.erf.doubleSpinBox_2.value(); MAOP = self.erf.doubleSpinBox_4.value()
            SF = self.erf.doubleSpinBox_5.value(); Axial_L = self.erf.doubleSpinBox_8.value(); Depth_P = self.erf.doubleSpinBox_9.value()
            if OD == 0 or WT == 0 or SF == 0:
                self.erf.lineEdit_2.setText("-"); self.erf.lineEdit_3.setText("-"); return
            flow_stress = 1.1 * SMYS; z_factor = (Axial_L**2) / (OD*WT); M = (1 + 0.8*z_factor) ** 0.5
            y = 1 - 2/3 * Depth_P / WT; z = 1 - 2/3 * Depth_P / WT / M; k = y / z
            S = (flow_stress * k) if z_factor <= 20 else (flow_stress * (1 - Depth_P/WT))
            EFP = (2 * S * WT) / OD; PSafe = EFP / SF if SF else 0
            if PSafe == 0:
                self.erf.lineEdit_2.setText("-"); self.erf.lineEdit_3.setText("-"); return
            ERFv = MAOP / PSafe; self.erf.lineEdit_2.setText(f"{ERFv:.2f}"); self.erf.lineEdit_3.setText(f"{PSafe:.2f}")
            import numpy as np
            def calc_B(d_over_t):
                if d_over_t >= 0.175:
                    B = np.sqrt(((d_over_t/(1.1*d_over_t-0.15))**2) - 1); return B if B <= 4 else 4
                return 4
            xs = np.linspace(0,1,100); ys = [calc_B(x) for x in xs]
            Xc = Axial_L/300; Yc = Depth_P/20; color = 'green' if Yc < calc_B(Xc) else 'red'
            fig = go.Figure(); fig.add_trace(go.Scatter(x=xs,y=ys,mode='lines',name='ASME B31G'))
            fig.add_trace(go.Scatter(x=[Xc],y=[Yc],mode='markers',marker=dict(color=color,size=10),name='Defect'))
            fig.update_layout(xaxis_title='Axial Length (mm)', yaxis_title='Peak Depth', height=450, width=1000)
            fp = resource_path('backend/files/ASME.html'); fig.write_html(fp); self.erf.web_viewERF.setUrl(QUrl.fromLocalFile(fp))
        for w in (self.erf.doubleSpinBox, self.erf.doubleSpinBox_3, self.erf.doubleSpinBox_2,
                  self.erf.doubleSpinBox_4, self.erf.doubleSpinBox_5, self.erf.doubleSpinBox_8, self.erf.doubleSpinBox_9):
            w.valueChanged.connect(update_result)
        update_result(); self.erf.show()
    ...
    def add_plot_custom(self):
        try:
            self.cplot_widget = customPlot(self.header_list); self.ui.graphLayout.addWidget(self.cplot_widget)
    ...
    def plot_c(self):
        try:
            y_label = self.cplot_widget.comboBox.currentText(); x_label = self.cplot_widget.comboBox_2.currentText()
            if x_label not in self.curr_data or y_label not in self.curr_data: raise ValueError("Selected labels are not in the current data.")
            x_data = self.curr_data[x_label]; y_data = self.curr_data[y_label]
            fig = go.Figure(); fig.add_trace(go.Scatter(x=x_data, y=y_data, mode='lines', name=y_label))
            ...
            self.cplot_widget.webviewCustom.setUrl(QUrl.fromLocalFile(fp)); self.web_view.setUrl(QUrl.fromLocalFile(fp))
        except Exception as e:
            self.open_Error(e)
    ...
    def add_plot_tele(self):
        try:
            ...
            self.tplot_widget = telePlot(tlist); self.ui.graphLayout.addWidget(self.tplot_widget)
            ...
            if len(tlist)>0:
                self.tplot_widget.comboBox.setCurrentIndex(1); self.plot_telemetry()
        except Exception as e:
            self.open_Error(e)
    ...
    def magnetisation(self):
        try:
            if not self.tplot_widget.checkBox.isChecked():
                fp = resource_path('backend/files/telemetryplot.html'); go.Figure().write_html(fp)
            else:
                filtered = [c for c in self.curr_data.columns if c.startswith('F')]; tele = self.curr_data[filtered]
                mag = tele.mean(axis=1) * 0.0004854; x = self.curr_data['ODDO1']; y = mag
                ...
                fp = resource_path('backend/files/magnetisation.html'); fig.write_html(fp)
            self.tplot_widget.webviewTele.setUrl(QUrl.fromLocalFile(fp)); self.web_view.setUrl(QUrl.fromLocalFile(fp))
        except Exception as e:
            self.open_Error(e)
    ...
    def plot_telemetry(self):
        try:
            ...
            filtered = [c for c in self.curr_data.columns if c.startswith('F')]; tele = self.curr_data[filtered]
            fig = go.Figure(); fig.add_trace(go.Scatter(x=tele.index, y=tele[param], mode='lines', name=param))
    ...
    def close_project(self):
        try:
            ...
            if tw is not None: tw.blockSignals(True)
            self.project_is_open = False; self.project_root = None; self.pkl_files = []
            self.hmap = self.hmap_r = self.lplot = self.lplot_r = self.pipe3d = self.heatmap_box = None
            self.curr_data = None; self.header_list = []; self.defect_df_view = None
            cb = self.ui.comboBoxPipe; cb.blockSignals(True); cb.clear(); cb.addItem("-Pipe-"); cb.blockSignals(False)
            self.model.clear(); self.ui.tableWidgetDefect.clear()
            self.web_view.setUrl(QUrl()); self.web_view2.setUrl(QUrl()); self.bottom_stack.setCurrentIndex(0)
            self._show_watermark(); self._toggle_plot_ui(False)
            if tw is not None:
                try: tw.setCurrentIndex(0)
                except Exception: pass
            self.btnDigsheetAbs.setEnabled(False); self.btnFullTable.setEnabled(False); self._update_project_actions()
            QMessageBox.information(self,"Project Closed","The project has been successfully closed.")
        except Exception as e:
            self.open_Error(e)
        finally:
            if tw is not None: tw.blockSignals(False)
    ...
    def open_digs(self):
        try:
            ...
            with open(pipe_tally_file,"wb") as f: pickle.dump(self.pipe_tally,f)
    ...
    def _show_disabled_digsheet_hint(self):
        QMessageBox.information(self,"Digsheet","Please choose <b>Absolute Distance</b> from the defect table below to generate the digsheet.")

    def _project_required_popup(self):
        QMessageBox.information(self,"Project Required","Please create project before proceeding further.")

    def _project_gate_targets(self):
        names = ["btnHeatmap","btnLinechart","btn3D","toolButtonHeatmap","toolButtonLine","toolButton3D"]
        widgets = [self.btnDigsheetAbs] + [getattr(self.ui, n, None) for n in names]
        return [w for w in widgets if w and hasattr(w, "mapFromGlobal")]

    def eventFilter(self, obj, ev):
        try:
            if obj is self.mid_tabbar and ev.type()==QEvent.Type.MouseButtonPress:
                if self._ui_ready and not self.project_is_open:
                    self._project_required_popup(); return True
            if ev.type()==QEvent.Type.MouseButtonPress:
                ...
                if btn and btn.isVisible() and not btn.isEnabled():
                    gp = ev.globalPosition().toPoint() if hasattr(ev,"globalPosition") else ev.globalPos()
                    if btn.rect().contains(btn.mapFromGlobal(gp)):
                        self._show_disabled_digsheet_hint(); return True
        except Exception:
            pass
    ...
    def open_Error(self, e):
        try:
            ...
            lay = QVBoxLayout(dlg); t = QTextEdit(); t.setReadOnly(True); t.setText(str(e))
            t.setStyleSheet("font-size:10pt;font-family:Consolas;color:#aa0000;")
            lay.addWidget(t); b = QPushButton("Close"); b.clicked.connect(dlg.accept); lay.addWidget(b); dlg.exec()
