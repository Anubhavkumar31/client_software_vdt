...
def pre_process_data(datafile, pipe_number, output_folder, total_sensors_hall, column_names, minute_sensors, degree_sensors):
    ...
    def add_sensor_keys(d):
        for e in d:
            new_dict = {**e}
            for i in range(1, total_sensors_hall):
    ...
    def create_time_dict():
        time_list = [timedelta(minutes=i * minute_sensors) for i in range(total_sensors_hall)]
...
def _process_one_pkl(pkl_path, output_folder):
    try:
        ...
        total_sensors_count_hall, total_sensors_count_proximity, column_names, minute_sensors, degree_sensors = count_pattern_minute_degree(pkl_path)
        print(f" total_sensors_count_hall: {total_sensors_count_hall}, total_sensors_count_proximity: {total_sensors_count_proximity} column_names: {column_names}, minute_sensors: {minute_sensors}, degree_sensors: {degree_sensors}")
        dfile = pre_process_data(data, pipe_number, output_folder, total_sensors_count_hall, column_names, minute_sensors, degree_sensors)
...
def count_pattern_minute_degree(datafile_path):
    ...
    pattern_proximity = re.compile(r'^F\d+P\d+$')
    ...
    count = len(matching_columns_proximity)
    count_hall = len(matching_columns_hall)
    print("mathcing columns hall : ", count_hall)
    print("mathcing columns proximity : ", count)
    minute_sensors_hall = 720 / count_hall 
    degree_sensors_hall = minute_sensors_hall / 2
    minute_sensors_proximity = 720 / count
    degree_sensors_proximity = minute_sensors_proximity / 2
    print(f"minute_sensors_hall: {minute_sensors_hall} : minute_sensors_proximity: {minute_sensors_proximity} \n degree_sensors_hall: {degree_sensors_hall} :: degree_sensors_proximity: {degree_sensors_proximity}")
    return count,count_hall, matching_columns_hall, minute_sensors_hall, degree_sensors_hall
