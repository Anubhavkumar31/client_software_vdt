...
from config.paths import Form, Window, resource_path
from utils.table import setup_table_scroll
from widgets.midbar import MidBarSplitter
from widgets.loading import ModernLoadingDialog
from workers.pipe_loader import PipeLoaderWorker
from models.pandas_model import PandasModel
from config.paths import ICON_ARROW_DOWN, _dump_tally_to_temp
...
""")
        ...
        import os
        print ("DEBUG ICON_ARROW_DOWN:", ICON_ARROW_DOWN)
        arrow_url = QUrl.fromLocalFile(ICON_ARROW_DOWN).toString()
        print("DEBUG arrow_url:", arrow_url)

        self.ui.comboBoxPipe.setEditable(True)
        import os
        print("DEBUG ICON_ARROW_DOWN:", ICON_ARROW_DOWN)

        # Convert path to forward slashes for Qt CSS compatibility
        if isinstance(ICON_ARROW_DOWN, str):
            arrow_path = ICON_ARROW_DOWN.replace("\\", "/")
        else:
            # Handle if ICON_ARROW_DOWN is a QUrl object
            arrow_path = str(ICON_ARROW_DOWN).replace("\\", "/")

        print("DEBUG arrow_path:", arrow_path)
    ...
    def open_project(self):
        try:
            ...
            if self.pkl_files:
                self.project_is_open = True
                self._setup_generate_button_styles()
                self._update_generate_actions()
                self._hide_create_project_message()
                self._toggle_plot_ui(True)
    ...
    * {{
    ...
    def open_Final_Report(self):
        if not self.project_is_open or not self.project_root:
            QMessageBox.warning(
                ...
                "3. Then try accessing Final Report again"
            )
            return
    ...
    def _setup_generate_button_styles(self):
        """Apply visual disabled styling while keeping actions enabled"""
        if not self.project_is_open:
            generate_style = """
                QMenu::item {
        ...
        if hasattr(self.ui, 'menuGenerate'):
            self.ui.menuGenerate.setStyleSheet(generate_style)
    ...
    def _update_generate_actions(self):
        ...
        if hasattr(self.ui, 'action_Final_Report'):
            self.ui.action_Final_Report.setEnabled(True)
        if hasattr(self.ui, 'Final_Report'):
            self.ui.Final_Report.setEnabled(True)
        if hasattr(self.ui, 'action_Preliminary_Report'):
            self.ui.action_Preliminary_Report.setEnabled(True)
        if hasattr(self.ui, 'action__pipetally'):
            self.ui.action__pipetally.setEnabled(True)
        if hasattr(self.ui, 'actionStandard'):
            self.ui.actionStandard.setEnabled(True)
