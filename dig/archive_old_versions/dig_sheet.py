...
#     dlg.grab_set()
#
#     dlg.resizable(False, False)  # Disable maximize/resize
#     dlg.protocol("WM_DELETE_WINDOW", lambda: None)  # Disable close (X)
#     dlg.attributes('-toolwindow', True)  # Hides minimize button on most systems
#
#
...
#     # unbind the global arrow keys used by the embedded viewer
#     try:
...
#     """Wipe the preview area and show a centered placeholder message."""
...
#         justify="center"
#     )
#     # Center it inside preview_holder and keep it centered on resize
...
#       section_id: 1=Feature Location on Joint, 2=Feature Description, 3=Pipe Location, 4=Client Description
...
def on_load_click():
    ...
    # FEATURE Display Logic (2 Up + 2 Down)
    feature_slots = [
        {"x": mid_x - 190, "arrow_x": mid_x - 200, "text_x": mid_x - 160, "source": features_upstream[::-1],
         "index": 1},
        {"x": mid_x - 90, "arrow_x": mid_x - 100, "text_x": mid_x - 60, "source": features_upstream[::-1], "index": 0},
        {"x": mid_x + 110, "arrow_x": mid_x + 120, "text_x": mid_x + 80, "source": features_downstream, "index": 0},
        {"x": mid_x + 210, "arrow_x": mid_x + 220, "text_x": mid_x + 180, "source": features_downstream, "index": 1},
    ...
    for slot in feature_slots:
        ...
        try:
            ...
            arrow_val = round(abs(float(upstream_weld_dist) - float(dist_val)), 2)
            pipe_canvas.create_line(arrow_x, mid_y - 95, arrow_x, mid_y - 65, arrow=tk.FIRST, fill="deepskyblue",
                                    width=2, tags="us_arrow")
            pipe_canvas.create_text(text_x, mid_y - 80, text=f"{arrow_val}(m)", font=("Arial", 9), tags="us_arrow")
        except:
            continue  # Skip if out of bounds or missing
    ...
    # BEND Display Logic (3 Up + 3 Down)
    bend_slots = [
        {"source": bends_upstream[::-1], "index": 2, "x_name": mid_x - 230, "x_dist": mid_x - 230, "x_lat": mid_x - 235,
         "x_lon": mid_x - 235, "tri_x": mid_x - 200, "arrow_text_x": mid_x - 215},
        {"source": bends_upstream[::-1], "index": 1, "x_name": mid_x - 140, "x_dist": mid_x - 140, "x_lat": mid_x - 135,
         "x_lon": mid_x - 135, "tri_x": mid_x - 110, "arrow_text_x": mid_x - 125},
        {"source": bends_upstream[::-1], "index": 0, "x_name": mid_x - 50, "x_dist": mid_x - 50, "x_lat": mid_x - 35,
         "x_lon": mid_x - 35, "tri_x": mid_x - 20, "arrow_text_x": mid_x - 35},
        {"source": bends_downstream, "index": 0, "x_name": mid_x + 55, "x_dist": mid_x + 55, "x_lat": mid_x + 50,
         "x_lon": mid_x + 50, "tri_x": mid_x + 110, "arrow_text_x": mid_x + 30},
        {"source": bends_downstream, "index": 1, "x_name": mid_x + 155, "x_dist": mid_x + 155, "x_lat": mid_x + 150,
         "x_lon": mid_x + 150, "tri_x": mid_x + 210, "arrow_text_x": mid_x + 130},
        {"source": bends_downstream, "index": 2, "x_name": mid_x + 255, "x_dist": mid_x + 255, "x_lat": mid_x + 250,
         "x_lon": mid_x + 250, "tri_x": mid_x + 310, "arrow_text_x": mid_x + 230},
    ]
    ...
    for slot in bend_slots:
        ...
        try:
            ...
            draw_triangle(slot["tri_x"], mid_y + 40)
            arrow_val = round(abs(float(upstream_weld_dist) - float(dist_val)), 2)
            pipe_canvas.create_text(slot["arrow_text_x"], mid_y + 35, text=f"{arrow_val}", font=("Arial", 9),
                                    tags="us_arrow")
            pipe_canvas.create_text(slot["arrow_text_x"], mid_y + 45, text="(m)", font=("Arial", 9), tags="us_arrow")
        except:
            continue
...
def open_save_dialog():
    ...
    opts.pack(pady=4)
    tk.Radiobutton(opts, text="PNG (image)", variable=mode_var, value="png", bg="white").grid(row=0, column=0, padx=10)
    tk.Radiobutton(opts, text="PDF (single page)", variable=mode_var, value="pdf", bg="white").grid(row=0, column=1,
                                                                                                    padx=10)
...
def reset_ui():
    ...
    # 4) Comment area
    try:
        comment_placeholder.config(text="")
    except:
        pass
...
try:
    # icon_path = os.getcwd() + "/dig" + "/digsheet_icon/"
    icon_path = resource_path("dig/digsheet_icon/")
    valve_img = ImageTk.PhotoImage(Image.open(icon_path + "valve.png").resize((18, 18)))
    bend_img = ImageTk.PhotoImage(Image.open(icon_path + "bend.png").resize((18, 18)))
    flange_img = ImageTk.PhotoImage(Image.open(icon_path + "flange.png").resize((18, 18)))
    flowtee_img = ImageTk.PhotoImage(Image.open(icon_path + "flowtee.png").resize((18, 18)))
...
def save_individual_section(section_id):
    ...
    try:
        ...
        if not filepath:
            return
        print(
            f"[{name}] base=({x0},{y0},{x1},{y1}) thresholds=({dx0},{dy0},{dx1},{dy1}) -> adjusted=({ax0},{ay0},{ax1},{ay1}) screen_h={root.winfo_screenheight()}")
...
def draw_pipe(pipe_canvas1, pipe_length, upstream, clock):
    ...
    try:
        ...
        pipe_canvas1.create_line(arrow1_start, arrow_y, arrow1_end, arrow_y, arrow=tk.LAST)
        pipe_canvas1.create_line(arrow1_end, arrow_y, arrow1_start, arrow_y, arrow=tk.LAST)
        pipe_canvas1.create_text((arrow1_start + arrow1_end) / 2, arrow_y - 10, text=f"{round(upstream, 2)} m",
                                 font=("Arial", 10))
        ...
        pipe_canvas1.create_line(arrow2_start, arrow_y, arrow2_end, arrow_y, arrow=tk.LAST)
        pipe_canvas1.create_line(arrow2_end, arrow_y, arrow2_start, arrow_y, arrow=tk.LAST)
        pipe_canvas1.create_text((arrow2_start + arrow2_end) / 2, arrow_y - 10, text=f"{remaining} m",
                                 font=("Arial", 10))
        ...
        # Color fill if in front (0–180), border if back (180–360)
        if 0 <= angle_deg <= 180:
            pipe_canvas1.create_rectangle(defect_x - 4, defect_y - 4, defect_x + 4, defect_y + 4, fill="orange",
                                          outline="black")
        else:
            pipe_canvas1.create_rectangle(defect_x - 4, defect_y - 4, defect_x + 4, defect_y + 4, outline="orange",
                                          width=2)
...
# --- Feature Description UI Block ---
feature_desc_frame = tk.Frame(scrollable_frame, bg="white", padx=5, pady=2, highlightbackground="black",
...
# --- Third Block Setup ---
third_frame = tk.Frame(scrollable_frame, bg="white", padx=10, pady=10, highlightbackground="black",
                       highlightthickness=1)
third_frame.pack(fill="both", padx=15, pady=4)
...
def main(tally_pkl, project_root=None):
    import sys

    # simulate CLI args (same as tera existing logic)
    sys.argv = [__file__, tally_pkl]
    if project_root:
        sys.argv.append(project_root)

    try:
        on_load_click()   # already tere code me hai
    except Exception as e:
