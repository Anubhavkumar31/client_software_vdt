...
tk.Button(input_frame, text="Load Excel", command=load_excel).grid(row=0, column=1, padx=5)
tk.Label(input_frame, text="Enter Defect S.no:", bg="white").grid(row=0, column=2, sticky="e")
defect_entry = tk.Entry(input_frame, width=10)
defect_entry.grid(row=0, column=3, padx=10)
...
def save_canvas_as_image():
    try:
        # Define the coordinates of the area to capture (the canvas)
        x1, y1, x2, y2 = canvas.bbox("all")  # Get the bounding box of all items on the canvas
        
        # Capture the image of the canvas
        img = ImageGrab.grab(bbox=(x1, y1, x2, y2))
        
        # Open the Save As dialog to allow the user to choose the file location
        file_path = filedialog.asksaveasfilename(
            defaultextension=".png",  # Default extension if the user doesn't specify one
            filetypes=[("PNG Files", "*.png"), ("JPEG Files", "*.jpg;*.jpeg"), ("All Files", "*.*")],  # Allow image formats
            title="Save Image As"
        )
        
        if file_path:  # If the user selected a file path (not canceled)
            # Save the captured image to the selected path
            img.save(file_path)
...
tk.Label(desc_frame, text="Pipe Description:", bg="white", fg="deepskyblue",
         font=("Arial", 10, "bold")).grid(row=0, column=0, columnspan=2, pady=(0, 10), sticky="ew")
...
feature_desc_frame = tk.Frame(scrollable_frame, bg="white", padx=5, pady=5, highlightbackground="black", highlightthickness=1)
...
third_frame.pack(fill="both", padx=15, pady=5)

# Title for the Third Block
tk.Label(third_frame, text="Pipe Location:", bg="white", fg="deepskyblue",
         font=("Arial", 10, "bold")).grid(row=0, column=0, columnspan=5,  sticky="ew")

# --- Sub-blocks Representation ---
pipe_canvas = tk.Canvas(third_frame, width=650, height=400, bg="white", highlightthickness=0)
...
for i in range(6):
    x1 = mid_x - 240 + i * 80
    x2 = x1 + 80
    pipe_canvas.create_rectangle(x1, mid_y + 120, x2, mid_y + 190, width=1)
