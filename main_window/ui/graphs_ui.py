...
from PyQt6.QtCore import Qt, QUrl
from pages.graphs_func import plot_erf, plot_psafe, plot_depth, plot_orientation
# from pages.Defects import (plot_metal_loss, plot_sensor_percentage, plot_temperature)
from pages.Defects import plot_metal_loss # Plot functions
import time 
...
class GraphApp(QWidget):
    def __init__(self, dataframe=None, project_root=None):
        super().__init__()
        self.df = dataframe
        ...
        self.graph_type_label = QLabel("Graph Type:")
        self.graph_type = QComboBox()
        self.graph_type.addItems(["", "Defects", "ERF", "Psafe", "Depth", "Orientation", "Temperature", "Sensor Loss","Magnetisation","Velocity"])
        self.graph_type.currentTextChanged.connect(self.on_graph_type_changed)
        self.graph_type.setToolTip("Choose the type of graph to display")
        top_bar.addWidget(self.graph_type_label)
        top_bar.addWidget(self.graph_type)
        ...
        self.feature_identification_label = QLabel("Feature Type:")
        self.feature_identification = QComboBox()
        self.feature_identification.addItems(["", "Corrosion", "MFG", "All"])
        self.feature_identification.setToolTip("Filter by defect feature (Corrosion, MFG, Both)")
        self.feature_identification.currentTextChanged.connect(self.on_feature_identification_changed)
        top_bar.addWidget(self.feature_identification_label)
        top_bar.addWidget(self.feature_identification)
        ...
        # Depth Bin
        self.depth_bin_label = QLabel("Depth Bin:")
        self.depth_bin = QComboBox()
        self.depth_bin.addItems(["All", "0–20%", "20–40%", "40–60%", "60–80%"])
        self.depth_bin.setToolTip("Filter graph by depth % bin")
        self.depth_bin.hide()
        self.depth_bin_label.hide()
        top_bar.addWidget(self.depth_bin_label)
        top_bar.addWidget(self.depth_bin)
    ...
    def on_graph_type_changed(self, text):
        ...
        if text == "Defects":
            ...
            self._set_view_visible(True)
        elif text in [ "Temperature", "Sensor Loss"]:
            pass  # no extra controls
        if text == "Depth":
            self.depth_bin.setCurrentText("All")  # 👈 reset every time Depth selected
            self.depth_bin.show()
            self.depth_bin_label.show()
        else:
            self.depth_bin.hide()
            self.depth_bin_label.hide()
        ...
        try:
            ...
            if graph_type == "Defects":
                if not feature and dimension == "":
                    QMessageBox.warning(self, "Missing Selection",
                                        "Select Feature Identification or Dimensional Classification.")
                    return
                fig, path = plot_metal_loss(self.df.copy(),
                                            feature_type=feature or None,
                                            dimension_class=dimension or None,
                                            return_fig=True)
                self.current_fig = fig
                self.browser.load(QUrl.fromLocalFile(path))
            ...
            #     self.browser.load(QUrl.fromLocalFile(path))
                ...
                try:
                    ...
                    img_path = os.path.join(self.project_root, "graphs", "sensor_loss.jpg").replace("/", "\\")

                    # img_path = os.path.join(self.project_root, "graphs","Sensor Loss", "sensor_loss.png")
                    # img_path = r"D:\pickle_6\graphs\Sensor Loss\Screenshot 2025-09-20 144012.png"
                    print("Loading Sensor Loss image from:", img_path)
                    self.browser.load(QUrl.fromLocalFile(img_path))
                except Exception as e:
                    print("Error loading Sensor Loss image:", e)

                # Save this path for later
                self.current_fig = None
                self.current_img_path = img_path

                self.save_btn.setEnabled(True)  # allow saving
                self.status_label.setText("✅ Sensor Loss graph loaded.")

            elif graph_type == "Magnetisation":
                img_path = os.path.join(self.project_root, "graphs", "magnetisation.jpg").replace("/", "\\")
                # img_path = r"D:\pickle_6\graphs\magnetization\Screenshot 2025-09-20 141752.png"
                self.browser.load(QUrl.fromLocalFile(img_path))
                ...
                self.save_btn.setEnabled(True)  # allow saving
                self.status_label.setText("✅ Magnetisation graph loaded.")

            elif graph_type == "Velocity":
                img_path = os.path.join(self.project_root, "graphs", "velocity.jpg").replace("/", "\\")
                # img_path = r"D:\pickle_6\graphs\Velocity\Screenshot 2025-09-20 141909.png"
                self.browser.load(QUrl.fromLocalFile(img_path))
                ...
                self.save_btn.setEnabled(True)  # allow saving
                self.status_label.setText("✅ Velocity graph loaded.")
            ...
            #     self.browser.load(QUrl.fromLocalFile(path))
                ...
                img_path = os.path.join(self.project_root, "graphs", "temperature.jpg").replace("/", "\\")
                # img_path = r"D:\pickle_6\graphs\Temperature\Screenshot 2025-09-20 144052.png"
                self.browser.load(QUrl.fromLocalFile(img_path))
                ...
                self.save_btn.setEnabled(True)  # allow saving
                self.status_label.setText("✅ Temperature graph loaded.")
    ...
    def save_graph(self):
        if (hasattr(self, 'current_fig') and self.current_fig is not None) or \
                (hasattr(self, 'current_img_path') and self.current_img_path):
            ...
            if file_path:
                try:
                    ...
                    # ✅ Handle Plotly vs Matplotlib vs Static Image
                    if self.current_fig is not None:
                        if hasattr(self.current_fig, "write_image"):  # Plotly
                            self.current_fig.write_image(file_path)
                        elif hasattr(self.current_fig, "savefig"):  # Matplotlib
                            self.current_fig.savefig(file_path, dpi=300)
                        else:
                            raise Exception("Unsupported figure type")
                    elif hasattr(self, "current_img_path") and self.current_img_path:
                        import shutil
                        shutil.copy(self.current_img_path, file_path)
